...
```python
# Compare the total amount associated to all disturbances
...
# load simulation outputs on Soil Organic Matter content 
...
soil_pools_libcbm = pools_libcbm[['identifier', 'timestep', 'area', 'above_ground_very_fast_soil', 'below_ground_very_fast_soil',
...
pools_cbm3.query("pool in @soil_pools")
...
### Sum for each pool at time step 0 for libcbm
...
soil_pools_libcbm_t0.set_index(['identifier', 'timestep', 'area']).sum()
```

```python
soil_pools_libcbm_t0.set_index(['identifier', 'timestep', 'area']).sum().sum()
```

## SOC by cbmcfs3

```python
so_me_pools_cbm3_t0 = soil_pools_cbm3.query("time_step == 0")
```
