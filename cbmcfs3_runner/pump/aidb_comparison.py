...
class CompareAIDB(object):
    ...
    def load_decay_rates(self):
        ...
        decay_cbm = decay_cbm[['pool_id', 'code', 'organic_matter_decay_rate', 'reference_temp',   'q10','max_decay_rate_soft',  'max_decay_rate_hard',  'prop_to_atmosphere' ]]
        ...
        # Join cbmcfs3 table with the maping table, then with the libcbm table
    ...
    def check_decay_parameters(self, threshold=1e-9):
        """Check that the absolute value of the difference between the turnover parameters is below 1e-9"""
        df_decay = self.load_decay_rates()
        all_equal = all(df_decay['diff'].abs() < threshold)
        if all_equal:
            print(f"{self.iso2_code} decay rates are identical in libcbm and cbmcfs3")
        else:
            problem_rows = df_decay[df_decay['diff'].abs() > threshold]
            msg = f"decay parameter mismatch in '{self.iso2_code}'\n{problem_rows}.\n"
            msg += "You can load the comparison table with: \n"
            msg += f"compare = CompareAIDB(cbmcfs3_continent.countries['{self.iso2_code}'])\n"
            msg += "df = compare.load_turnover_parameters()\n"
            msg += f"problem_rows = df[df['diff'].abs() > {threshold}]"
