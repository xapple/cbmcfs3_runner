...
class DisturbanceMaker(object):
    ...
    @property_cached
    def gftm_irw_proxy(self):
        # Load #
        df = self.country.demand.gftm_irw.copy()
        # Artificially increase demand for scenarios #
        df['value_ob'] = df['value_ob'] * self.irw_artificial_ratio
        df['value_ub'] = df['value_ub'] * self.irw_artificial_ratio
        # Return #
        return df

    @property_cached
    def gftm_fw_proxy(self):
        # Load #
        df = self.country.demand.gftm_fw.copy()
        # Artificially increase demand for scenarios #
        df['value_ob'] = df['value_ob'] * self.fw_artificial_ratio
        df['value_ub'] = df['value_ub'] * self.fw_artificial_ratio
        # Return #
        return df
    ...
    def dist_irw_converted(self):
        ...
        weight in tonnes and  of carbon to m^3 over bark
        to the demand volume in m^3 over bark for each year.
        ...
        df = df.outer_join(self.gftm_irw_proxy, ['step', 'hwp'])
        # Return #
        return df
    ...
    def dist_fw_converted(self):
        ...
        # Outer join to capture all demand values,
        # even if step or con_broad is not present in dist anymore
        df = df.outer_join(self.gftm_fw_proxy, ['step', 'hwp'])
