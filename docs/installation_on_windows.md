...
The approach was to install the cbmcfs3_runner python package. Then work through the error message and solve the missing dependencies.
...
Display environmental variables from within python:
...
As visible from `~/repos/cbmcfs3_runner/setup.py` under the `install_requires` variable.
...
Clone the `cbmcfs3_data` repository make it available to python by entering that system environment variable CBMCFS3_DATA as explained above.
...
* SIT from zip, create directory SIT in C:\CBM\SIT
...
You might need the [MS Access 2013 runtime](https://www.microsoft.com/en-us/download/details.aspx?id=39358) depending on your version of office as explained on this [Microsoft trouble shooting page](https://docs.microsoft.com/en-us/office/troubleshoot/access/cannot-use-odbc-or-oledb).
...
Or change the default user name to os.getlogin() inside the 2 places in the code that are creating an instance of an access database. Pass os.getlogin() as the username argument.
...
runner not finding aidb_eu, several times - needed to reinstall CBM_CFS3 (in C:/Program Files (x86), run as administrator) order to overcome the issue

### Access security warning

A security warning can prevent SIT from reading the AIDB Access database. This prevents CBMCFS3 from running. List of forums with potential workarounds:

- [Changing the security level in the registry](https://stackoverflow.com/a/15673444/2641825)

- [VB script to add to the trust centre or changing the security level](https://answers.microsoft.com/en-us/msoffice/forum/all/how-to-disable-message-for-microsoft-access/1969a6e0-bfae-4910-a36b-35de9e5b10ab)

- [4 different ways to fix MS access security notice](http://www.accessrepairnrecovery.com/blog/fix-microsoft-access-security-notice)

- [General error Unable to open registry key Temporary (volatile) Ace DSN for process ...](https://stackoverflow.com/a/26317631/2641825) "This is the top-level error message produced by the Access Database Engine (a.k.a. "ACE") ODBC driver when the current process is unable to open the Access database file ". The answer goes on to propose 5 different solutions depending on the reason that could generate this error.
